...
#			return False
	...
	def checkmate(self, color):
		...
		king = self.table[slef.kingPos[color]]
		cmap = king.moveMap()
		...
		for y in range(0,8):
			for x in range(0,8):
				piece = self.table[y][x]
				if piece != ' ' and piece.color == hostileColor:
					for kingy in range(0,8):
						for kingx in range(0,8):
							if piece.validateMove(kingy,kingx) and piece.validateTake(kingy,kingx):
								cmap[kingy][kingx] = str(piece)
