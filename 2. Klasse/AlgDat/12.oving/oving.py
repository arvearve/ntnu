...
def minste_avstand(strenger):
    ...
    for x in range(len(strenger)):
        current = strenger[x]
        current_lengde_sum = 0
        for streng in strenger:
            current_lengde_sum += avstand(current, streng)
     #   print 'min_sum_avstand: ', min_sum_avstand
     #   print 'current_lengde_sum: ', current_lengde_sum
        min_sum_avstand = min(min_sum_avstand, current_lengde_sum)

    return min_sum_avstand
    # SLUTT IKKE-UTDELT KODE
